...
export class UserService extends CommonService {
  ...
  public static async forgotPassword(_username: string) {
    ...
    // Store userDetails.rowCount in a separate variable
    let rowCount = userDetails?.rowCount ?? 0 // Use optional chaining and the nullish coalescing operator
    ...
    try {
      ...
      rowCount = updatedUser?.rowCount ?? 0
      if (rowCount <= 0) {
  ...
  public static async getPermissions(usernameOrEmail: string) {
    ...
    const rowCount = result?.rowCount ?? 0 // Use optional chaining and the nullish coalescing operator
