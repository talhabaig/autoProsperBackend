  ...
  public static async getEnumInfo(enumClassName: string){
    switch (enumClassName) {
      case 'CarEssentialCriteria':
        const carEssentialEnumValues = Object.values(CarEssentialCriteria).filter(value => typeof value === 'number') as CarEssentialCriteria[];
      ...
      case 'PrefferedCarBrand':
        const prefferedBrandEnumValues = Object.values(PrefferedCarBrand).filter(value => typeof value === 'number') as PrefferedCarBrand[];
        ...
        return prefferedBrandEnumValues.map(value => ({ value, displayName: prefferedCarBrandDisplayNames[value] || '' }));

      case 'PrefferedSeats':
        const prefferedSeatsEnumValues = Object.values(PrefferedSeats).filter(value => typeof value === 'number') as PrefferedSeats[];
        ...
        return prefferedSeatsEnumValues.map(value => ({ value, displayName: prefferedSeatsDisplayNames[value] || '' }));
    
      case 'PrefferedVehicle':
        const prefferedVehicleEnumValues = Object.values(PrefferedVehicle).filter(value => typeof value === 'number') as PrefferedVehicle[];
        const prefferedVehicleDisplayNames: Record<PrefferedVehicle, string> = PrefferedVehicleDisplayNames;
        return prefferedVehicleEnumValues.map(value => ({ value, displayName: prefferedVehicleDisplayNames[value] || '' }));
      case 'PrefferedRidePower':
        const prefferedRidePowerEnumValues = Object.values(PrefferedRidePower).filter(value => typeof value === 'number' ) as PrefferedRidePower[];
        const prefferedRidePowerDisplayNames: Record<PrefferedRidePower, string> = PrefferedRidePowerDisplayNames;
        return prefferedRidePowerEnumValues.map(value => ({ value, displayName: prefferedRidePowerDisplayNames[value] || '' }))
