...
// Type guard function
function isCustomWinstonTransport(transport: winston.transport): transport is CustomWinstonTransport {
  return 'name' in transport
}
...
const filterTransport = winston.format((info, opts): any => {
  ...
  const transport = logger.transports.find((t) => isCustomWinstonTransport(t) && t.name === opts.target)

  // Determine if the fetched transport was one of the intended targets when logging was invoked.
  // @ts-ignore
  // eslint-disable-next-line
  const abort = Array.isArray(info.targets) && !(info.targets.length === 0 || info.targets.includes('all') || info.targets.includes((transport as CustomWinstonTransport).name))
